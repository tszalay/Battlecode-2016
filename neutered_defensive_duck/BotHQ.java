...
public class BotHQ extends Bot {
    ...
    private static void setRallyLoc() throws GameActionException {
        ...
        // clear the defensive target from the message board if the tower is dead
        if (!Defense.isDefensiveLocTowerStillAlive()) {
    		if (! (MessageBoard.NUM_DEFENDERS.readInt()==0) ) {// if it hasn't already been cleared
