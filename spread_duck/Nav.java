...
public class Nav extends Bot {
    ...
    private static Direction findBugMoveDir() throws GameActionException {
        ...
        for (int i = 8; i-- > 0;) {//spin around and look for obstacle!
            if (canMove(dir)) return dir;//didn't see an obstacle ahead, move in look dir (90 from last bug move into wall) and keep the counter running
            dir = (bugWallSide == WallSide.LEFT ? dir.rotateRight() : dir.rotateLeft());
            bugMovesSinceSeenObstacle = 0;//ran into obstacle again
        }
        return null;//you are surrounded, give a null direction
    }
    ...
    private static void bugMove(Direction dir) throws GameActionException {
        if (move(dir)) {
            ...
            else bugLookStartDir = dir.rotateRight().rotateRight();//90deg or 45? to right of dir you just went
        }
    }
    ...
    public static void goTo(MapLocation theDest, NavSafetyPolicy theSafety) throws GameActionException {
        ...
        if (rc.getType() != RobotType.DRONE) { // BFS avoids voids, but drones need not
            if (tryMoveBfs()) {
            	rc.setIndicatorString(1, "bfs");
            	return;
            }
        }
        rc.setIndicatorString(1, "bug");
        bugMove();
    }
}
