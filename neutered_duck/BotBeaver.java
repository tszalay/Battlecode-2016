...
//        Debug.indicate("build", 0, "buildLoc = " + (buildLoc == null ? "null" : buildLoc.toString()));

        if (here.isAdjacentTo(buildLoc)) {
//            Debug.indicate("build", 1, "adjacent");
            boolean builtSomething = tryBuildSomethingAtBuildLoc();
            if(!builtSomething) {
                if(buildLoc != null && rc.isCoreReady()) {
                    rc.mine();
                }
            }
        } else {
...
//                    Debug.indicate("build", 2, "found opportunistic build loc: " + buildLoc.toString());
                    return;
                }
            }
//            Debug.indicate("build", 1, "going to build loc");
            goToBuildLoc();
        }
    }
    ...
    private static boolean okToBuildAt(MapLocation loc) {
        ...
        RobotInfo[] nearbyEnemies = rc.senseNearbyRobots(loc, 15, them);
    ...
    private static boolean tryBuildSomethingAtBuildLoc() throws GameActionException {
        if (!rc.isCoreReady()) return false;

        RobotType buildingType = MessageBoard.DESIRED_BUILDING.readRobotType();
        if (buildingType == RobotType.HQ) return false; // HQ means don't build anything

        if (rc.getTeamOre() < buildingType.oreCost) return false;
...
//            Debug.indicate("build", 2, "getting off this square");
            getOffThisSquare();
        } else {
            ...
            Nav.goTo(buildLoc, safetyPolicy);
        }
    }
