...
public class BotLauncher extends Bot {
        ...
        if (rc.isCoreReady()) {
            if (kiteBack()) {
                ...
                if (Clock.getRoundNum() - lastKitedBackRound > 8 || rc.getMissileCount() > 0) {
            		...
            		rc.setIndicatorString(0, "normal launcher");
                    rc.setIndicatorString(1, "Target = " + rallyLoc.x + "," + rallyLoc.y);
                    
                    MapLocation snipeLoc = MessageBoard.SNIPE_LOC.readMapLocation();                  
                    if (isSniper()) {
                		 rc.setIndicatorString(0, "sniper launcher");
                    	 rc.setIndicatorString(1, "Target = " + snipeLoc.x + "," + snipeLoc.y);
                    	 rallyLoc = snipeLoc;
                    }
    ...
    private static boolean isSniper() throws GameActionException {
    	// one quarter of launchers are selected to try to snipe enemy towers
    	if (rc.getID()%4 == 0) {
    		return true;
    	}
    	else
