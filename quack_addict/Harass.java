...
public class Harass extends Bot {
    ...
    public static void doHarass() throws GameActionException {
        ...
        // do we have a target?
        int targetID = MessageBoard.ENEMY_RAGE_ID.readInt();
        RobotInfo enemyTarget = null;
        
        if (rc.canSenseRobot(targetID))
        	enemyTarget = rc.senseRobot(targetID);

        
        RobotInfo[] nearbyEnemies = rc.senseNearbyRobots(64, them);
        ...
        if (enemyTarget == null)
        {
        	for (RobotInfo enemy : nearbyEnemies)
        	{
        		...
        		// miner? keep, unless we find a drone
        		else if (enemy.type == RobotType.MINER && enemyTarget == null)
        		{
        			enemyTarget = enemy;
        		}
        	}
        	// did we find one?
        	if (enemyTarget != null)
        		MessageBoard.ENEMY_RAGE_ID.writeInt(enemyTarget.ID);
        }
        
        // so let's see if we have a drone nearby that we micro-attack
        boolean shadowEnemyHarassers = true;
        if (enemyTarget != null && enemyTarget.type == RobotType.DRONE && here.distanceSquaredTo(enemyTarget.location) < 64)
        {
        ...
        if (rc.isCoreReady()) {
        	...
        	if (enemyTarget != null)
            	dest = enemyTarget.location;
        	
        	int droneLeaderID = MessageBoard.DRONE_HARASS_LEADER.readInt();
        	if (dest == null && rc.canSenseRobot(droneLeaderID))
        	{
        		// if we're farther, we become leader
        		RobotInfo leader = rc.senseRobot(droneLeaderID);
        		if (here.distanceSquaredTo(ourHQ) > leader.location.distanceSquaredTo(ourHQ))
        			MessageBoard.DRONE_HARASS_LEADER.writeInt(rc.getID());
        		else
        			dest = leader.location;
        	}
            
        	// still nada, enemy base
        	if (dest == null)
        		dest = theirHQ;

            NavSafetyPolicy safetyPolicy = new SafetyPolicyReallyAvoidAllUnits(enemyTowers, nearbyEnemies);
            Nav.goTo(dest, safetyPolicy);
        }
    }
}
