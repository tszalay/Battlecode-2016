...
public class RobotPlayer {
	...
	static double anti_launcher_supply = 1500;
	...
	static final int TANK_SUPPLY_MOD = 2; // for supply
	...
	public static void run(RobotController robotc)
	{
		...
		try {
			...
			case DRONE:
				...
				if(droneNum <= 8 && !Consts.LAUNCHER_ENEMY)
...
//					myState = UnitState.CONVOY;
	...
	{
		if(myType==RobotType.TANK && rc.getID()%TANK_SUPPLY_MOD == 0)
			return; // supply hoarding tanks do not do computations
	...
	static void doDrone()
	{
		...
		try {
			...
			case LOAD:
				...
				if (Consts.LAUNCHER_ENEMY && rc.getSupplyLevel() > anti_launcher_supply)
					scoutState = ScoutState.HARASS;
				break;
...
//			System.out.println("pathable set to " + Integer.toBinaryString(GRID_MASK&(norms&~(prevpathable))));
	...
	static void transferSupplies(double supplyFactor) throws GameActionException
	{
		...
		case MINER:
			for(RobotInfo bot : nearbyAllies)
			{
				...
				{
					if(bot.supplyLevel<lowestSupplyRating) // only go to lowest supplied
					{
						lowestSupplyRating = bot.supplyLevel;
						transferAmount = mySupply*0.5;
						suppliesToThisLocation = bot.location;
					}
				}
			}
			if(suppliesToThisLocation!=null && transferAmount>500)
		...
		case TANK:
			if(rc.getHealth() < 50)
			{
				...
				for(RobotInfo bot : nearbyAllies)
				{
					if(bot.type==RobotType.TANK && bot.supplyLevel<lowestSupplyRating && bot.health > botHealth) // only go to lowest supplied
					{
						lowestSupplyRating = bot.supplyLevel;
						botHealth = bot.health;
						transferAmount = mySupply;
						suppliesToThisLocation = bot.location;
					}
				}
				if(suppliesToThisLocation!=null && botHealth > 0)
					rc.transferSupplies((int)transferAmount, suppliesToThisLocation);
			}
			else // hoard and dump near battle
			{
				//find rage target
				int ragePriority = 0;
				int curTargetID = rc.readBroadcast(rageTargetChan);
				RobotInfo rageTarget = null;
				int distanceFromAction = 1000;
				if (rc.canSenseRobot(curTargetID))
				{
					ragePriority = Consts.ragePriorities[rc.senseRobot(curTargetID).type.ordinal()];
					rageTarget = rc.senseRobot(curTargetID);
					distanceFromAction = myLocation.distanceSquaredTo(rageTarget.location);
				}
				else // if we can't sense it, fight might be over, check around
				{
					RobotInfo[] nearbyEnemies = rc.senseNearbyRobots(100, enemyTeam);
					// fight is over, go back to hoarding
					if(rc.getID()%TANK_SUPPLY_MOD != 0 && mySupply > 100 && nearbyEnemies.length == 0)
					{
						// not a hoarder, dump to hoarder if there's one around
						for(RobotInfo bot : nearbyAllies)
						{
							if(bot.type==RobotType.TANK && bot.supplyLevel<lowestSupplyRating && rc.getID()%TANK_SUPPLY_MOD == 0) // go to lowest supplied hoarder
							{
								lowestSupplyRating = bot.supplyLevel;
								transferAmount = mySupply; // all of it!
								suppliesToThisLocation = bot.location;
							}
						}
				...
				// if we're near fight and supplied, dump it
				if(mySupply > 100 && ragePriority > 2 && distanceFromAction < 100)
				{
					for(RobotInfo bot : nearbyAllies)
					{
						if(bot.type==RobotType.TANK && bot.supplyLevel<lowestSupplyRating) // only go to lowest supplied
						{
							lowestSupplyRating = bot.supplyLevel;
							transferAmount = mySupply*0.5;
							suppliesToThisLocation = bot.location;
					...
					if(suppliesToThisLocation!=null && transferAmount>50)
						rc.transferSupplies((int)transferAmount, suppliesToThisLocation);
				}
			}
			break;
		...
		default:
			boolean tankTransfer = false;
			boolean minerTransfer = false;
			for(RobotInfo bot : nearbyAllies)
			{
				if(bot.type==RobotType.TANK && (bot.ID % TANK_SUPPLY_MOD)==0 && bot.supplyLevel<5000) // only some tanks
				{
					transferAmount = Math.min(mySupply*0.9, 4000);
					suppliesToThisLocation = bot.location;
					if(suppliesToThisLocation!=null && transferAmount>100)
					{
						rc.transferSupplies((int)transferAmount, suppliesToThisLocation);
						tankTransfer = true;
						break;
				...
				else if(bot.type==RobotType.MINER && bot.supplyLevel<5000)
				{
					transferAmount = Math.min(mySupply*0.9, 4000);
					suppliesToThisLocation = bot.location;
					if(suppliesToThisLocation!=null && transferAmount>100)
				...
				else if(bot.type==RobotType.SUPPLYDEPOT || bot.type==RobotType.TANKFACTORY || bot.type==RobotType.HELIPAD || bot.type==RobotType.BARRACKS || bot.type==RobotType.MINERFACTORY)
				{
					if(bot.supplyLevel<lowestSupplyRating) // only go to lowest supplied
					{
						lowestSupplyRating = bot.supplyLevel;
			...
			if(!tankTransfer && !minerTransfer && suppliesToThisLocation!=null && transferAmount>100)
			{
	...
	static int getEnemyOutrangeMoves()
	{
		RobotInfo[] bots = rc.senseNearbyRobots(myType.sensorRadiusSquared, enemyTeam);
		MapLocation myloc = rc.getLocation();
