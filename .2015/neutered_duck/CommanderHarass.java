...
//                    Debug.indicate("micro", 0, "no nearby enemies, shadowing an enemy at long range");
        ...
        if (numEnemiesAttackingUs > 0) {
            ...
            if (canTakeMultipleEnemies(enemiesAttackingUs, numEnemiesAttackingUs)) {
                ...
                if (here.distanceSquaredTo(bestEnemyToAttack.location) > 8 && tryMoveTowardLocationSafely(bestEnemyToAttack.location, nearbyEnemies)) {
...
//        Debug.indicate("micro", 1, "don't need to flee");
    ...
    static void tryShootLowestHealthNearbyEnemy(RobotInfo[] nearbyEnemies) throws GameActionException {
        if(!rc.isWeaponReady()) return;
        
        RobotInfo enemyToShoot = null;
        double minHealth = 1e99;
        for(RobotInfo enemy : nearbyEnemies) {
            if(RobotType.COMMANDER.attackRadiusSquared >= here.distanceSquaredTo(enemy.location)) {
        ...
        if(enemyToShoot != null) {
            rc.attackLocation(enemyToShoot.location);
        }
    }
...
//        Debug.indicate("micro", 2, "flash cooldown = " + rc.getFlashCooldown());
        ...
        if (inHealingState) {
            tryShootLowestHealthNearbyEnemy(nearbyEnemies);
            if (rc.isCoreReady()) {
