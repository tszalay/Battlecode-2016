                        ...
                        if (nearbyEnemies.length == 0) { // stop while we kill the enemy
        					...
        					for (RobotInfo enemy : nearbyEnemies) {
        						if (enemy.type==RobotType.LAUNCHER) {
        							if (Rage.tryBroadcastNewRageLocation(enemy)) rc.setIndicatorString(2, "Broadcast.");
        							break;
        						}
        					}
                        }
    ...
    static boolean tryLaunchMissile() throws GameActionException {
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type != RobotType.MISSILE) {
            	//Rage.tryBroadcastNewRageLocation(enemy);
            	//rc.setIndicatorString(2, "Trying to broadcast.");
