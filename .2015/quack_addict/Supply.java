...
public class Supply extends Bot {
    ...
    public static void runSupplies() throws GameActionException {
        if (here.equals(supplyRunnerLastLoc)) {
            supplyRunnerTurnsSinceMove++;
            if (supplyRunnerTurnsSinceMove >= 50 && here.distanceSquaredTo(ourHQ) > GameConstants.SUPPLY_TRANSFER_RADIUS_SQUARED) {
        ...
        if (onSupplyRun) {
            ...
            if (supplyRunnerNeed > 0) {
                ...
                double supplyNeededForRun = 3 * RobotType.DRONE.supplyUpkeep * Math.sqrt(ourHQ.distanceSquaredTo(supplyRunnerDest));
        ...
        if (supplyRunnerRetreatIfNecessary(nearbyEnemies, enemyTowers)) return;
    ...
    private static boolean needToRetreat(RobotInfo[] nearbyEnemies) {
        for (RobotInfo enemy : nearbyEnemies) {
            switch (enemy.type) {
                case MISSILE:
                    if (here.distanceSquaredTo(enemy.location) <= 15) return true;
                    break;

                case LAUNCHER:
                    if (here.distanceSquaredTo(enemy.location) <= 24) return true;
                    break;

                default:
                    if (enemy.type.attackRadiusSquared >= here.distanceSquaredTo(enemy.location)) return true;
                    break;
            }
        }
        return false;
    ...
    private static boolean supplyRunnerRetreatIfNecessary(RobotInfo[] nearbyEnemies, MapLocation[] enemyTowers) throws GameActionException {
        if (!needToRetreat(nearbyEnemies)) return false;
    ...
    public static void hqGiveSupply() throws GameActionException {
        ...
        {
        	resupplyDrone = rc.senseRobot(resupplyDroneID);
        }
        else
        {
            for (RobotInfo ally : nearbyAllies) {
        ...
        if (resupplyDrone != null && resupplyDrone.location.distanceSquaredTo(here) < GameConstants.SUPPLY_TRANSFER_RADIUS_SQUARED) {
            // Debug.indicate("supply", 1, "transferring " + hqSupplyReservedForResupplyDrone + " to resupply drone");
            rc.transferSupplies((int) hqSupplyReservedForResupplyDrone, resupplyDrone.location);
