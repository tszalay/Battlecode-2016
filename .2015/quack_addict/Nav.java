...
class SafetyPolicyReallyAvoidAllUnits extends Bot implements NavSafetyPolicy {
    MapLocation[] enemyTowers;
    RobotInfo[] nearbyEnemies;

    public SafetyPolicyReallyAvoidAllUnits(MapLocation[] enemyTowers, RobotInfo[] nearbyEnemies) {
        this.enemyTowers = enemyTowers;
        this.nearbyEnemies = nearbyEnemies;
    }
    ...
    public boolean isSafeToMoveTo(MapLocation loc) {
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            switch (enemy.type) {
                case MISSILE:
                    if (loc.distanceSquaredTo(enemy.location) <= 64) return false;
                    break;
