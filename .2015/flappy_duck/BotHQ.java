...
public class BotHQ extends Bot {
    ...
    private static void setRallyLoc() throws GameActionException {
        ...
        if (!attackMode) {
            ...
            snipeLoc = new MapLocation((ourHQ.x + theirHQ.x) / 2, (ourHQ.y + theirHQ.y) / 2);
        } else {
            boolean goForHQ;
            ...
            if (goForHQ) {
                ...
                for (MapLocation tower : enemyTowers) {
                    ...
                    // otherwise, snipe tower is the one farthest from their HQ (Try to avoid having the same as rallyLoc)
                    else { 
                    
                    	if (snipeLoc == null || snipeLoc == rallyLoc || theirHQ.distanceSquaredTo(tower) > theirHQ.distanceSquaredTo(snipeLoc)) {
                        snipeLoc = tower;               
                    	}
                    }
                }
                ...
                for (MapLocation tower : ourTowers) {
                	...
                	if (defendLoc == null || rc.senseRobotAtLocation(tower).health < minHealth) {
                		defendLoc = tower;
                		minHealth = rc.senseRobotAtLocation(tower).health;
                	}
                	
                	if (rc.senseRobotAtLocation(tower).health == 1000) defendLoc = rallyLoc;;
                }
                
                // if losing, use snipe launchers to defend
                if (numTowers < enemyTowers.length) snipeLoc = defendLoc;
            }      
        }


        MessageBoard.RALLY_LOC.writeMapLocation(rallyLoc);
        rc.setIndicatorString(0, "RallyLoc = " + rallyLoc.x + "," + rallyLoc.y);
        rc.setIndicatorDot(rallyLoc, 255, 0, 0);
        
        if (snipeLoc == null) snipeLoc = rallyLoc;
        MessageBoard.SNIPE_LOC.writeMapLocation(snipeLoc);
        rc.setIndicatorString(1, "SnipeLoc = " + snipeLoc.x + "," + snipeLoc.y);
        rc.setIndicatorDot(snipeLoc, 0, 0, 255);
    }
